...
public class PlaybackService extends Service implements GpxSaxParserListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (timeFromIntent != null && !"".equalsIgnoreCase(timeFromIntent)) {
            ...
            queue.start(delayTimeOnReplay,getApplicationContext());
        }
...
//            if (delay > 0) {
        ...
        try {
            Log.d(LOG, "Sending Point in:" + ( System.currentTimeMillis()) + "ms    "+locCount);

//
        } catch (Exception e) {
            Log.d("Loc Toast",e.toString());
        }
        SendLocationWorker worker = new SendLocationWorker(mLocationManager, item, PROVIDER_NAME, 10000);
